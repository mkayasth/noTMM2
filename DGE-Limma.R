...
## dge between just high risk NO_TMM and ALT/Telomerase.
...
# Filtering for FDR < 0.01.
noTMM_candidates <- noTMM_results[round(noTMM_results$adj.P.Val, 2) <= 0.05 & (noTMM_results$logFC >= 0.5 | noTMM_results$logFC <= -0.5), ]
...
# Storing significant t-test results where FDR is less than 0.01.
t_test_results_sig2 <- t_test_results2[round(t_test_results2$fdr_t_test, 2) <= 0.05, ]
...
# Storing significant t-test results where p-value is less than 0.01.
t_test_results_sig3 <- t_test_results3[round(t_test_results3$fdr_t_test, 2) <= 0.05, ]
...
##### 
candidate_genes <- intersect(t_test_results_sig2$Gene, t_test_results_sig3$Gene)
